...
from easyhec.utils.preview import RealSensePreview
from easyhec.utils.utils_3d import merge_meshes


@dataclass
class PiperXArgs(Args):
    """Calibrate an external RealSense camera against a Piper-X robot arm.

    This script is eye-to-hand only: the RealSense is treated as a fixed external
    camera, and each sample records the current Piper-X joint feedback plus one
    RGB frame while you manually place the robot in teaching mode.

    While capturing, a live preview window stays open so you can confirm the arm
    remains inside the camera field of view. The preview pauses briefly at each
    capture and resumes right after.
    """

    output_dir: str = "results/piper_x"
    use_previous_captures: bool = False
    """Reuse previously captured images/link poses/masks if they exist."""

    can_name: str = "can_left_slave"
    """Activated CAN interface name used by piper_sdk. Must be the follower arm
    (slave port), matching cmds/piper_x_sdk/can_config.sh naming:
    can_left_slave / can_right_slave are followers, can_*_main are leaders."""

    realsense_camera_serial_id: str = "346522071769"
    ...
    paper_extrinsic_ros_path: Optional[str] = None
    """Optional paper calibration result used to initialize Camera<-Piper-X base in ROS convention."""

    paper_to_piper_base_x: float = 0.0
    paper_to_piper_base_y: float = 0.0
    paper_to_piper_base_z: float = 0.0
    """Paper-origin translation in the Piper-X base frame, in meters."""
...
def import_piper_api():
    try:
        ...
        try:
            ...
            raise ImportError(
                "piper_sdk is required for Piper-X calibration. Install it with "
...
def create_piper(can_name: str, allow_mode_switch: bool):
    ...
    if allow_mode_switch and hasattr(piper, "EnableArm"):
        print("allow_mode_switch=True; enabling Piper-X arm feedback via EnableArm(7)")
        piper.EnableArm(7)
    return piper
...
def resolve_ros_package_urdf(urdf_path: Path, tmp_dir: Path) -> Path:
    """Rewrite package://agx_arm_description/... mesh URIs into local paths.

    The Piper-X URDF was exported from SolidWorks for a ROS package and still
    points at package-relative mesh assets. In this repository the STL meshes
    live next to the URDF under meshes/. The URDF references .dae files for
    visuals which are not present here, so both visual and collision meshes are
    redirected to the local .stl files before loading with urchin.
    """
    urdf_text = urdf_path.read_text()
    mesh_root = (urdf_path.parent / "meshes").resolve()
    urdf_text = urdf_text.replace(
        "package://agx_arm_description/agx_arm_urdf/piper_x/meshes/dae/",
        f"{mesh_root.as_posix()}/",
    )
    urdf_text = urdf_text.replace(
        "package://agx_arm_description/agx_arm_urdf/piper_x/meshes/",
        f"{mesh_root.as_posix()}/",
    )
    urdf_text = urdf_text.replace(".dae", ".stl")
...
def wait_for_manual_capture(sample_idx: int, total_samples: int):
    prompt = (
        f"\nPlace the Piper-X at manual pose {sample_idx}/{total_samples}, "
        "make sure the arm is stationary and visible in the preview window, "
...
def capture_manual_samples(
    ...
    print(
        "Live preview is running. Drag the arm and confirm it stays visible in the "
        "preview window before each capture. Press Enter in the terminal to capture."
    )
    ...
    for sample_idx in range(args.num_manual_samples):
        ...
        if not camera.running:
            raise RuntimeError(
                "Preview window was closed before all samples were captured. "
                f"Got {sample_idx}/{args.num_manual_samples}."
            )
...
def align_loaded_link_poses_dataset(
    ...
    if max(mesh_link_indices) >= link_poses_dataset.shape[1]:
        raise ValueError(
            "Cached link_poses_dataset does not match the current Piper-X URDF layout. "
...
def paper_to_piper_base_rotation() -> np.ndarray:
    # Piper-X base axes from the paper frame. Adjust this matrix to match your
    # physical setup if you provide a paper calibration via paper_extrinsic_ros_path.
    return np.array(
        [
            [0.0, 1.0, 0.0],
            [-1.0, 0.0, 0.0],
...
def paper_to_piper_base_transform(args: PiperXArgs) -> np.ndarray:
...
def resolve_initial_extrinsic_guess(args: PiperXArgs) -> np.ndarray:
    ...
    if paper_extrinsic_path is not None and paper_extrinsic_path.exists():
        ...
        print("Initial extrinsic guess from manual PiperXArgs values")
    ...
    print(f"Camera<-Piper-X base (ROS):\n{repr(initial_extrinsic_guess_ros)}")
    print(f"Camera<-Piper-X base (OpenCV):\n{repr(initial_extrinsic_guess)}")
...
def main(args: PiperXArgs):
    if len(ARM_JOINT_NAMES) != 6:
        raise ValueError("ARM_JOINT_NAMES must contain exactly 6 Piper-X joints.")
    ...
    with TemporaryDirectory(prefix="easyhec_piper_x_") as tmp_dir_str:
            ...
            if args.use_previous_captures and link_poses_path.exists() and image_dataset_path.exists():
                ...
                if intrinsic_path.exists():
                    ...
                    print(
                        ...
                        "Connecting to the RealSense to read intrinsics."
                    )
                    camera = RealSensePreview(
                        serial_id=args.realsense_camera_serial_id,
                        width=args.camera_width,
                        height=args.camera_height,
                        fps=args.camera_fps,
                    )
                    intrinsic = camera.read_intrinsic()
                    camera.close()
            else:
